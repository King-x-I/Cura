...
interface ProviderProfileFields extends ConsumerProfileFields {
  age: number | null;
...
export const useProfileCompletion = (): ProfileCompletionHook => {
  ...
  const updateProfileCompletion = async (fieldName: string, value: any) => {
    ...
    try {
      ...
      if (userType === "consumer") {
        ...
        calculateCompletion(updatedProfileData as ProviderProfileFields, getProviderRequiredFields(updatedProfileData as ProviderProfileFields));
      }
    } catch (error) {
  ...
  const checkProviderProfile = async () => {
    ...
    setProfileData(data);
    const profile = data as ProviderProfileFields;
