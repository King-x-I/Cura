...
import { MapPin, Calendar as CalendarIcon, Clock, MessageSquare, Loader2 } from "lucide-react";
import { Textarea } from "@/components/ui/textarea";

const formSchema = z.object({
  childAge: z
    .string()
    .min(1, { message: "Please select child age" }),
  location: z
    .string()
  ...
  medicalNeeds: z
    .string()
    .min(1, { message: "Please select if there are medical needs" }),
...
const NannyService = () => {
  ...
  const onSubmit = (values: z.infer<typeof formSchema>) => {
    ...
    // Mock API call to book a nanny
    setTimeout(() => {
      setBooking(true);
      toast.success("Nanny booking requested!");
      setIsLoading(false);
    }, 2000);
  };
    ...
    return (
      <DashboardLayout userType="consumer">
        <Card className="max-w-lg mx-auto">
          <CardHeader>
            <CardTitle className="text-center">Finding a Nanny</CardTitle>
          </CardHeader>
          <CardContent className="space-y-6 flex flex-col items-center">
            ...
            <div className="text-center space-y-2">
              <p className="text-lg">
                We're looking for a suitable nanny for your child
              </p>
              <p className="text-gray-500">
                This won't take long. You'll receive a notification once a nanny accepts your request.
              </p>
            </div>
            <div className="bg-gray-50 p-4 rounded-lg w-full">
              <div className="space-y-2">
                <div className="flex gap-2">
                  <div className="text-pink-500 font-medium">Child Age:</div>
                  <div>{form.getValues("childAge")}</div>
                </div>
                <div className="flex gap-2">
                  <div className="text-pink-500 font-medium">Location:</div>
                  <div>{form.getValues("location")}</div>
                </div>
                <div className="flex gap-2">
                  <div className="text-pink-500 font-medium">Date:</div>
                  <div>{format(form.getValues("date"), "MMMM d, yyyy")}</div>
                </div>
                <div className="flex gap-2">
                  <div className="text-pink-500 font-medium">Time:</div>
                  <div>{form.getValues("time")}</div>
                </div>
                <div className="flex gap-2">
                  <div className="text-pink-500 font-medium">Duration:</div>
                  <div>{form.getValues("duration")}</div>
                </div>
                <div className="flex gap-2">
                  <div className="text-pink-500 font-medium">Medical Needs:</div>
                  <div>{form.getValues("medicalNeeds")}</div>
                </div>
              </div>
            </div>
  ...
  return (
    <DashboardLayout userType="consumer">
      <div className="max-w-lg mx-auto">
        <h1 className="text-3xl font-bold mb-6">Book a Nanny</h1>
              ...
              <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-6">
                    ...
                    <FormItem>
                      ...
                      <FormControl>
                        <div className="relative">
                          ...
                          <Textarea 
                            placeholder="Any special requirements or instructions for the nanny..."
                ...
                <div className="bg-gray-50 p-4 rounded-lg">
                  ...
                  <div className="text-sm text-gray-600">
                    Base rate: $20-25/hour depending on experience level and requirements.
                    <br />
                    Final price will be confirmed when a nanny accepts your booking.
                  </div>
                </div>
                
                <Button type="submit" className="w-full" disabled={isLoading}>
                  {isLoading ? "Processing..." : "Request a Nanny"}
                </Button>
              </form>
            </Form>
          </CardContent>
